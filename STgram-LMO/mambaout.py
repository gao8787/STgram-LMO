...
class KANHead(nn.Module):
    """ MLP classification head
    """
    def __init__(self, dim, num_classes=1000, act_layer=nn.SiLU, mlp_ratio=4,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), head_dropout=0., bias=True):
        super().__init__()
        self.act = act_layer()
        # add KAN layer
        self.KAN = KANLayer(in_dim=dim,out_dim=num_classes,num=7, k=5)
        self.head_dropout = nn.Dropout(head_dropout)
...
#  add
class VMlpHead(nn.Module):
    """ V-MLP classification head
    """
    def __init__(self, dim, num_classes=1000, mlp_ratio=4,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), head_dropout=0.2, bias=True):
        super().__init__()
        hidden_features = int(mlp_ratio * dim)
        self.fc1 = nn.Linear(dim, hidden_features, bias=bias)
        self.alpha = nn.Parameter(torch.ones(1))  # Learnable parameter for Swish-1 activation
        #self.act = nn.SiLU()  # Swish-1 activation function
        self.act = nn.Sigmoid()  # Swish-1 activation function
        self.norm = norm_layer(hidden_features)
    ...
    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x) * self.alpha  # Apply learnable slope to the activation function
        x = self.norm(x)
        x = self.dropout(x)
        x = self.fc2(x)
        return x
class CombinedHead(nn.Module):
    """ Combined MLP and V-MLP classification head
    """
    def __init__(self, dim, num_classes=1000, act_layer=nn.SiLU, mlp_ratio=4,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), head_dropout=0., bias=True):
        super().__init__()
        self.KAN_head = KANHead(dim, num_classes, act_layer, mlp_ratio, norm_layer, head_dropout, bias)
        self.VMlp_head = VMlpHead(dim, num_classes, mlp_ratio, norm_layer, head_dropout, bias)

    def forward(self, x):
        x1 = self.KAN_head(x)
        x2 = self.VMlp_head(x)
        return x1 + x2  # Or any other way you want to combine the outputs
...
class MambaOut(nn.Module):
    ...
    """
    def __init__(self, in_chans=3, num_classes=128,
                 depths=[3, 3, 9, 3],
                 #depths=[3, 4, 27, 3],
                 dims=[96, 192, 384, 576],   #tiny
                 #dims=[48, 96, 192, 288],
        ...
        self.fc_out = nn.Linear(128, 81920)   #modified 40960 81920
