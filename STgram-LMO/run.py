...
sep = os.sep
from lr_scheduler import CosineWarmupLR
...
def main(args):
    ...
    if not cuda:#or device_ids  is None
        ...
        args.device = torch.device('cuda:0')  #{device_ids[0]}
    ...
    # optimizer & scheduler
    #optimizer = torch.optim.LAMB(net.parameters(), lr=float(args.lr))
    #optimizer = torch.optim.SGD(net.parameters(), lr=float(args.lr))
    #optimizer = torch.optim.Adam(net.parameters(), lr=float(args.lr))
    # best
    #optimizer = torch.optim.AdamW(net.parameters(), lr=float(args.lr), weight_decay=0.001, amsgrad=True) #, weight_decay=0.0005, amsgrad=True, fused=True  0.001
    #optimizer = SGDP(net.parameters(), lr=float(args.lr), momentum=0.9, weight_decay=0.0005, nesterov=True)
    optimizer = AdamP(net.parameters(), lr=float(args.lr), weight_decay=0.0005, nesterov=True)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=0.1*float(args.lr))
    #scheduler = CosineWarmupLR(optimizer, args.batch_size, args.epochs, args.lr, target_lr=0.001, warmup_epochs=20, warmup_lr=0.00001, last_iter=-1)
    # trainer
