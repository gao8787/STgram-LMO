...
def get_latent_features(version, machine_type, epoch=120):
    ...
    cuda = param['cuda']
    # load model
    model_path = os.path.join('./', model_dir, version, 'model', f'{epoch}_checkpoint.pth.tar')
    net = SimCLRv2()
    ft_net = SimCLRv2_ft(net, 41)
    ft_net = torch.nn.DataParallel(ft_net, device_ids=[0])
    # ft_net.load_state_dict(
    #     torch.load(model_path, map_location=lambda storage, loc: storage.cuda() if cuda else 'cpu')['clf_state_dict'])
    model_dict = torch.load(model_path, map_location=lambda storage, loc: storage.cuda() if cuda else 'cpu')
    # ft_net.load_state_dict(model_dict['clf_state_dict'])
    # model = ft_net.module
    if cuda:
        model.cuda(1)
...
if __name__ == '__main__':
    version = 'STgram-MFN(m=0.7,s=30)'
    machine_type = 'fan'
    view = '2D'
    save_path = os.path.join('./runs', version, f't-SNE_{view}_{machine_type}_small.svg')

    data, id_labels, anomaly_labels, label_desc = get_data(version, machine_type, epoch='100')
