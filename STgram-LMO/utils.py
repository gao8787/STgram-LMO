    ...
    with open(file_path, "w", encoding='utf-8') as f:
        for key, value in data.items():
            if isinstance(value, torch.device):
                data[key] = str(value)  # 将设备对象转换为字符串
...
class Wave2Mel2(object):
    def __init__(self, sr,
        ...
        self.wavelet_transform = torchaudio.transforms.WaveletTransform(wavelet=wavelet, mode=mode)
        self.amplitude_to_db = torchaudio.transforms.AmplitudeToDB(stype='power')

    def __call__(self, x):
        wavelet_coeffs = self.wavelet_transform(x)
        return self.amplitude_to_db(wavelet_coeffs)

class Wave2Wavelet(object):
